...
stack_block::stack_block() : size(0), size_elector(0) { }

void stack_block::inc_electors() 
{
...
void stack_block::inc_data(int count) 
{
...
void stack_block::push(const byte &value)
{
	if (data == nullptr) 
	{
		data = new byte[++size];
...
void stack_block::push(const word &value)
{
	if (data == nullptr) 
	{
		size += 4;
...
void stack_block::push(const dword &value)
{
	if (data == nullptr) 
	{
		size += 4;
...
void stack_block::kill_electors() 
{
		byte *temp_elector = new byte[size_elector];
...
void stack_block::dec_data(int count)
{
	byte *temp_data = new byte[size];
...
void stack_block::kill_elector(int number) 
{
...
int stack_block::make_dword() 
{
	dword dw(data[size - 4], data[size - 3], data[size - 2], data[size - 1]);
	int temp = dw.convert_to_int();
	return temp;
}

int stack_block::make_word() 
{
...
int stack_block::elections(int electors_choise) 
{
	int temp(0);
...
void stack_block::print_stack()
{
	for (int i = size - 1; i >= 0; i--)
		std::cout << "data[" << i << "] : " << data[i] << std::endl;
}

void stack_block::print_electors()
{
